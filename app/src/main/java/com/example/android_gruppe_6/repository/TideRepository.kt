...
class TideRepository(private val database: HarborsDatabase) {
    ...
    suspend fun getDbHarborWithTide(harborName: String): List<HarborWithTide> {
    ...
    // Get tide from harbor
    suspend fun getDbTide(harborName: String): List<TideData> {
        var response: List<DbTide>
        withContext(Dispatchers.IO){
            response = database.harborDao.getTide(harborName)
        }
        return response.map { TideData(
            harbor = it.harbor,
            year = it.year,
            month = it.month,
            day = it.day,
    ...
    // Get tide one day and harbor
    suspend fun getDbTideOneDay(harborName: String, day: Int): List<TideData> {
        var response: List<DbTide>
        withContext(Dispatchers.IO){
            response = database.harborDao.getTideOneDay(harborName, day)
        }
        return response.map { TideData(
            harbor = it.harbor,
            year = it.year,
    ...
    // Get tide from harbor today
    suspend fun getDbTideToday(harborName: String): List<TideData>{
        var response: List<DbTide>

        withContext(Dispatchers.IO){
            response = database.harborDao.getTideOneDay(
                harborName,
